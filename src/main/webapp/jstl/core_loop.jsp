...
<%--
	...
	step : 증감 값(default 1)
	var : 인덱스 변수 or 향상된 for문에서 사용 될 경우 collection 객체에서 꺼낸 값 설정
	varStatus : loop var 
		index - 현재 인덱스 값
		count - loop 실행 횟수
		begin, end - loop의 begin, end 값
		
 --%>
 	<h3>일반 for 문 형태</h3>
	<c:forEach begin="0" end="10" var="i" varStatus="loop">
		${i}번 : 반복시킬 문장  ${loop.index} / ${loop.count}<br>
	</c:forEach>
	...
	<h3>map 타입 루프를 통해 담겨진 값 확인하기</h3>
	<%
	...
	var 속성을 적용한 경우 : var 속성으로 저장만 된다(el을 통해 활용) <c:url value="./registUser.jsp" var="url"/><br>
	${url }	
