...
<body>
	...
	<%--
		...
		2. test속성안에 공백을 넣으면 정상적으로 판단되지 않음
		<c:if test="${userid} == 'brown'"> X
		<c:if test="${userid == 'brown'} "> O
		
		
		<%
			if(((String)page.Context.getAttribute("userid")).equals("brown"){
	...
	<h3>choose : 일반 if, else if, else</h3>
 	...
 	<c:choose>
 		<c:when test="${userid =='sally'}">샐리 사용자 입니다</c:when>
