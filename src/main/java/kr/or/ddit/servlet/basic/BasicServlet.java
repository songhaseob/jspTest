...
public class BasicServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 만약에 log lovel을 debug보다 높은 레벨로 설정할 경우 
		...
		// servlet을 통해 응답으로 행성하는 html을 동적으로 변경 ==>
		// 						동적(servlet/jsp를 배우는 이유)
		
		// servlet의 라이프 사이클
		// init() ==>         service()              ==> destroy()
		// 로딩시 최초 1회    사용자가 요청을 할때 마다       서버 종료 or reload
		
		// init 메소드는 해당 서블릿에서 사용하여 자원을 초기화 할때 사용
		...
		// localhost/basicServlet ==> Basic Servlet의 service 메소드를 통해 응답생성
		// localhost/index.jsp ==> server 설정에 있는 web.xml로 등록된
		...
		PrintWriter pw = resp.getWriter(); //writer객체를 얻는다
