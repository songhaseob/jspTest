...
public class LoginController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * logger.debug("req.getParameterValues(\"userid\":"); for(String userid :
		 * req.getParameterValues("userid")) { logger.debug(userid); }
		 * 
		 * // 요청에 담긴 파라미터 이름을 확인 // hasMoreElements() 반환하는 타입이 true,false
		 * logger.debug("요청에 담긴 파라미터 이름 확인");
		 * logger.debug("req.getParameterNames(\"\":{}"); Enumeration<String>
		...
		// userid가 brown일 때 비밀번호가 brownpass 인 경우에 한해 로그인 성공되었다고 판단
		// 그 외 경우는 로그인 실패로 간주
		...
		// 로그인 실패시 : login.jspf로 redirect(문법적으로 배우기 위해, 올바른 상황에 대해서는 추후 다시)
		// redirect : 클라이언트에게 정해진 주소로 재요청 할 것을 지시
		//            redirect 응답을 받은 클라이언트는 재요청 주소로 서버에게 요청한다
		//            원 요청 + 재 요청 : 요청이 총 2번 필요
		//            reponse.sendRedirect("클라이언트가 새롭게 요청할 주소")
		...
		// 로그인 성공
		//로그인 성공 ==> service를 통해 데이터 베이스에 저장된 값과 일치 할때
